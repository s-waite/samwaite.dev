...
const BouncingText = ({ text }: { text: string }) => {
    const letters = text.split("").map((letter, index) => (
        <div
            key={index}
            className="will-change-auto group-hover:animate-bounce inline-block"
    ...
    return <div className='group will-change-auto'>{letters}</div>;
};
        ...
        <html lang="en" className={poppins.className}>
            <body className='m-0 p-0 absolute top-0'>
                <nav className='max-h-screen fixed w-[300px] py-6 pl-6 sm:px-4 sm:pb-0 overflow-hidden sm:block sm:sticky sm:top-0 sm:w-screen sm:bg-white sm:z-50 sm:h-max'>
                    ...
                    <div className='hover:animate-bounce'>test</div>
                    <BouncingText text=" test" />
                    <div>
